...
export default  function ItemsPage( ) {
  ...
  const id = params?.index; // ✅ Extracting `index` param
...
useEffect(() => {
  ...
  async function fetchItems() {
    try {
      const res = await fetch(`/api/updateentry/${id}`);
...
{/** */}
  ...
  const [keywords, setKeywords] = useState(blog.seo[0].seokeywords);


  const [file, setFile] = useState(null);
  const [customName, setCustomName] = useState((blog.image).replace("/uploads/","")); // Store custom name
  const [altname,setAltname]=useState(blog.imagealt)//altname
  const [message, setMessage] = useState("");
  const [ext, setExt] = useState("");

  const [seodes,setSeodes]=useState(blog.seo[0].description);
  const [seoauthor,setSeoauthor]=useState(blog.seo[0].author);
  ...
  <div style={{
    ...
    <div className="file-upload">
      ...
      <Image src={blog.image} alt="hello" quality={10} width={170} height={100}/>
      <input type="file" onChange={handleFileChange} />
